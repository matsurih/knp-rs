...
use crate::{check_feature, Chi_quote_end_matrix, Chi_quote_start_matrix, FEATUREptr, fprintf, free, Language, memset, MRPH_DATA, Outfp, Quote_matrix, SENTENCE_DATA, sprintf, stderr, strcat, strcmp, strdup, strlen};
use crate::ctools::{malloc_data, OptChiPos, string_length};
use crate::structs::QUOTE_DATA;
use crate::tools::{OptChiGenerative, OptDisplay};

#[no_mangle]
pub static mut quote_data: QUOTE_DATA = QUOTE_DATA { in_num: [0; 40], out_num: [0; 40] };

#[no_mangle]
pub unsafe extern "C" fn init_quote(mut sp: *mut SENTENCE_DATA) {
...
#[no_mangle]
pub unsafe extern "C" fn check_quote(mut sp: *mut SENTENCE_DATA) -> libc::c_int {
    /*
      "．．「○○」．．．「××」．．"  
      "．．「○○「×××」○○」．．" 
      "．．「○○○○○○○○○○○○"  
...
/*==================================================================*/
#[no_mangle]
pub unsafe extern "C" fn mask_quote(mut sp: *mut SENTENCE_DATA)
/*==================================================================*/
{
...
/*==================================================================*/
#[no_mangle]
pub unsafe extern "C" fn mask_quote_for_chi(mut sp: *mut SENTENCE_DATA)
/*==================================================================*/
{
    let mut i: libc::c_int = 0; /* １文節だけの括弧は無視 */
    let mut j: libc::c_int = 0;
    ...
    while quote_data.in_num[k as usize] >= 0 as libc::c_int {
        ...
        if !(start == end) {
            if OptChiPos == 0 &&
                !check_feature((*(*sp).bnst_data.offset(start as isize)).f,
                                       b"PU\x00" as *const u8 as
                                           *const libc::c_char as
                                           *mut libc::c_char).is_null() &&
                !check_feature((*(*sp).bnst_data.offset(end as isize)).f,
                                       b"PU\x00" as *const u8 as
                                           *const libc::c_char as
                                           *mut libc::c_char).is_null() ||
                OptChiPos != 0 {
...
#[no_mangle]
pub unsafe extern "C" fn quote(mut sp: *mut SENTENCE_DATA) -> libc::c_int {
    let mut quote_p: libc::c_int = 0 as libc::c_int;
    init_quote(sp);
    if Language != 2 as libc::c_int ||
        Language == 2 as libc::c_int && OptChiGenerative == 0 {
        quote_p = check_quote(sp);
        if quote_p != 0 {
            /* 鈎括弧の検出 */
            if quote_p == -(1 as libc::c_int) { return quote_p; }
            if OptDisplay == 3 as libc::c_int && Language != 2 as libc::c_int {
                print_quote();
            }
            if Language != 2 as libc::c_int {
                mask_quote(sp);
...
/*==================================================================*/
#[no_mangle]
pub unsafe extern "C" fn add_comment(mut sp: *mut SENTENCE_DATA, mut add_string: *mut libc::c_char) {
    if !(*sp).Comment.is_null() {
        /* 既存のコメントと結合 */
...
#[no_mangle]
pub unsafe extern "C" fn process_input_paren(mut sp: *mut SENTENCE_DATA, mut paren_spp: *mut *mut SENTENCE_DATA) -> libc::c_int {
    ...
    return if paren_num == 0 as libc::c_int || paren_num >= 100 as libc::c_int ||
        ...
        *paren_spp =
            malloc_data((::std::mem::size_of::<SENTENCE_DATA>() as
        ...
        while i < paren_num {
            ...
            *fresh1 =
                malloc_data(strlen((*sp).KNPSID).wrapping_add(4 as libc::c_int
                    as
                    libc::c_ulong),
                                    b"process_input_paren\x00" as *const u8 as
                                        *const libc::c_char as *mut libc::c_char) as
            ...
            *fresh2 =
                malloc_data(strlen(b"\xe6\x8b\xac\xe5\xbc\xa7\xe5\xa7\x8b:\xef\xbc\x88 \xe6\x8b\xac\xe5\xbc\xa7\xe7\xb5\x82:\xef\xbc\x89 \xe6\x8b\xac\xe5\xbc\xa7\xe4\xbd\x8d\xe7\xbd\xae:\x00"
                    as *const u8 as
        ...
        while i < (*sp).Mrph_num {
            ...
            char_pos +=
                string_length((*m_ptr.offset(i as isize)).Goi2.as_mut_ptr());
            i += 1
        }
...
/*==================================================================*/
#[no_mangle]
pub unsafe extern "C" fn prepare_paren_sentence(mut sp: *mut SENTENCE_DATA,
                                                mut paren_sp:
                                                *mut SENTENCE_DATA)
/*==================================================================*/
{
