...
use crate::{case_print, Dpnd_matrix, ErrorComment, Mask_matrix, match_matrix, path_matrix, PM_Memo, Quote_matrix, restrict_matrix, tools, tree_conv, WarningComment};
use crate::case_print::print_pa_structure;
use crate::consts::VERBOSE1;
use crate::ctools::{assign_cfeature, check_feature, exit, Language, malloc_data, OptAnalysis, Outfp, stderr};
...
use crate::regexp::FORM;
use crate::structs::{MRPH_DATA, TOTAL_MGR};
use crate::tools::{BNST_DATA, CLASS, CLASS_num, FEATURE, FILE, katakana2hiragana, MRPH_DATA, OptArticle, OptCKY, OptCopula, OptDisplay, OptEllipsis, OptExpress, OptInput, OptNbest, OptNE, OptPostProcess, OptRecoverPerson, PARA_DATA, PARA_MANAGER, PrintNum, realloc_data, SENTENCE_DATA, size_t, TAG_DATA, time_t, TOTAL_MGR, TYPE, VerboseLevel};
use crate::tree_conv::{init_bnst_tree_property, make_dpnd_tree};
...
pub unsafe extern "C" fn pp2mrph(mut pp: *mut libc::c_char,
 ...
 /*==================================================================*/
 {
    ...
    sprintf(mrph_buffer.as_mut_ptr(),
            b"%.*s\x00" as *const u8 as *const libc::c_char, pp_len, pp);
    hira_pp =
        katakana2hiragana(mrph_buffer.as_mut_ptr() as *mut libc::c_uchar) as
            *mut libc::c_char;
    hinsi_id =
        get_hinsi_id(b"\xe5\x8a\xa9\xe8\xa9\x9e\x00" as *const u8 as
                         *const libc::c_char);
    sprintf(mrph_buffer.as_mut_ptr(),
            b"%s %s %s \xe5\x8a\xa9\xe8\xa9\x9e %d \xe6\xa0\xbc\xe5\x8a\xa9\xe8\xa9\x9e %d * 0 * 0 NIL\x00"
                as *const u8 as *const libc::c_char, hira_pp, hira_pp,
            hira_pp, hinsi_id,
            get_bunrui_id(b"\xe6\xa0\xbc\xe5\x8a\xa9\xe8\xa9\x9e\x00" as
                              *const u8 as *const libc::c_char, hinsi_id));
...
pub unsafe extern "C" fn print_M_bnst(mut sp: *mut SENTENCE_DATA,
 ...
 /*==================================================================*/
 {
    ...
    if len > space {
        ...
        i = 0 as libc::c_int;
        while i < space - len { fputc(' ' as i32, Outfp); i += 1 }
    }
...
pub unsafe extern "C" fn print_line(mut length: libc::c_int,
 ...
 /*==================================================================*/
 {
    let mut i: libc::c_int = 0;
    i = 0 as libc::c_int;
    while i < length - 1 as libc::c_int { fputc('-' as i32, Outfp); i += 1 }
    if flag != 0 {
        fputc(')' as i32, Outfp);
...
pub unsafe extern "C" fn print_para_manager(mut sp: *mut SENTENCE_DATA,
 ...
 /*==================================================================*/
 {
    let mut i: libc::c_int = 0;
    i = 0 as libc::c_int;
    while i < level * 5 as libc::c_int { fputc(' ' as i32, Outfp); i += 1 }
    i = 0 as libc::c_int;
...
pub unsafe extern "C" fn mylog(mut n: libc::c_int) -> libc::c_int 
 /*==================================================================*/
 {
    ...
    i = 0 as libc::c_int;
    while i < n { num = num * 2 as libc::c_int; i += 1 }
    return num;
}
/*==================================================================*/
#[no_mangle]
...
pub unsafe extern "C" fn show_sexp(mut ptr: *mut BNST_DATA,
 ...
 /*==================================================================*/
 {
    let mut i: libc::c_int = 0;
    i = 0 as libc::c_int;
    while i < depth { fputc(' ' as i32, Outfp); i += 1 }
    fprintf(Outfp, b"(\x00" as *const u8 as *const libc::c_char);
    if (*ptr).para_top_p as libc::c_int ==
           (0 as libc::c_int == 0) as libc::c_int {
        if !(*ptr).child[1 as libc::c_int as usize].is_null() &&
 ...
 /*==================================================================*/
 {
    ...
    */
    ...
    if OptExpress == 4 as libc::c_int {
        ...
        /* 形態素のtree出力 */
        if make_dpnd_tree(sp) != 0 {
            tree_conv::bnst_to_mrph_tree(sp); /* 形態素の木へ */
            print_kakari(sp, OptExpress, eos_flag);
        } else { print_eos(eos_flag); }
    ...
    if case_print_flag != 0 && OptArticle == 0 &&
        ...
        case_print::print_case_result(sp, Sen_Num);
        /* 次の解析のために初期化しておく */
...
/*==================================================================*/
#[no_mangle]
pub unsafe extern "C" fn do_postprocess(mut sp: *mut tools::SENTENCE_DATA)
 /*==================================================================*/
 {
    /* 後処理 */
    if make_dpnd_tree(sp) != 0 {
        tree_conv::bnst_to_tag_tree(sp); /* タグ単位の木へ */
