...
use crate::ctools::{assign_cfeature, check_feature, malloc_data, stderr};
use crate::feature::{check_str_type, delete_cfeature};
use crate::{_FEATURE, BNST_DATA, context, entity, FEATURE, mention, MENTION_MGR, MRPH_DATA, sentence_data, tnode_b};
...
use crate::structs::{CF_ALIGNMENT, entity_manager};
...
use crate::types::{CASE_FRAME, CF_PRED_MGR, CFLIST, ENTITY, MENTION, SENTENCE_DATA, TAG_DATA};
...
pub unsafe extern "C" fn set_mention_from_coreference(mut tag_ptr: *mut TAG_DATA, mut mention_ptr: *mut MENTION) {
    ...
    if strchr((*(*mention_ptr).entity).name.as_mut_ptr(), ':' as i32).is_null() && { cp = check_feature((*tag_ptr).f, b"NE\x00" as *const u8 as *const libc::c_char as *mut libc::c_char);!cp.is_null() } {
        strcpy(temp.as_mut_ptr(), cp);
        abbreviate_NE(temp.as_mut_ptr());
        strcpy((*(*mention_ptr).entity).name.as_mut_ptr(), temp.as_mut_ptr().offset(strlen(b"NE:\x00" as *const u8 as *const libc::c_char) as isize));
        name_change_flag = 1 as libc::c_int
...
#[no_mangle]
pub unsafe extern "C" fn search_hypo_entity(mut sp: *mut SENTENCE_DATA,
                                            mut tag_ptr: *mut TAG_DATA,
 ...
 -> libc::c_int {
    ...
    if sscanf(token,
              b"%[^/]/%c/%[^/]/%d/%d/\x00" as *const u8 as
                  *const libc::c_char, rel.as_mut_ptr(),
              &mut type_0 as *mut libc::c_char, entity_name.as_mut_ptr(),
              &mut tag_num as *mut libc::c_int,
              &mut sent_num as *mut libc::c_int) == 0 {
        return 0 as libc::c_int
    }
    ...
    if i < (*(*tag_ptr).tcf_ptr).cf.element_num {
        ...
        let mut functional_tag_cf: CASE_FRAME =
            CASE_FRAME{type_0: 0,
                       ...
                       samecase: [[0; 2]; 24],
                       cf_align:
                           [CF_ALIGNMENT{cf_id: 0 as *mut libc::c_char,
                                         aligned_case: [[0; 2]; 24],}; 5],
 ...
 -> libc::c_int {
    ...
    if OptAnaphora & 16 as libc::c_int != 0 {
        ...
        while i < 3 as libc::c_int {
            ...
            if ellipsis_correct_ctm[i as usize].score ==
                ...
                copy_ctm(&mut *ellipsis_correct_ctm.as_mut_ptr().offset(i as
                                                                            isize),
                         ctm_ptr_0);
    ...
    if gs_ctm_ptr.is_null() {
        ...
        strcpy((*tag_ptr).mention_mgr.cf_id.as_mut_ptr(),
               (*ellipsis_result_ctm[0 as libc::c_int as
                                         usize].cf_ptr).cf_id.as_mut_ptr());
...
pub unsafe extern "C" fn all_sentence_anaphora_analysis(mut sp:
 ...
 /*==================================================================*/
 {
    ...
    loop  {
        ...
        sen_idx = 1 as libc::c_int;
        while sen_idx < sen_max { anaphora_analysis(sen_idx); sen_idx += 1 }
